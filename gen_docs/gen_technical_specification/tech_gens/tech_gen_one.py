...
class DataPreprocessingModuleSpecGenerator:
        ...
        self.prompts = {
            ...
            "system_overview": PromptTemplate(
                ...
                template=(
                    """
                    {system_message}
                    请提供一个{product_name}的系统概述，包括其主要功能和用途, 不少于3000字。
            ...
            "system_architecture": PromptTemplate(
                ...
                template=(
                    """
                    {system_message}
                    请详细描述{product_name}的系统架构, 不少于3000字。
            ...
            "technical_specification": PromptTemplate(
                ...
                template=(
                    """
                    {system_message}
                    请详细描述功能“{func_name}”的技术规范段落, 不少于5000字。
                    ...
                    以下是两个示例，请根据功能的类别去参考， 如果是平台功能，参考平台类示例，如果是模型相关功能，参考第二个模型类示例。
                    ...
                    4.1 算法集成与优化
                        ...
                        提供一套综合的时间序列分析与大模型融合算法包，专为企业级应用设计。

                        多模型集成：支持传统时间序列模型（如ARIMA、ETS）、深度学习模型（如LSTM、Transformer）、以及基于大模型（LLM）的时间序列增强方案。

                        自适应模型切换：提供动态模型选择与融合机制，基于数据特性和业务需求自动选择最佳算法。

                        预训练与微调：利用大模型（如GPT、T5）进行时间序列数据特征提取，并结合迁移学习对特定场景进行微调，提升预测精度。

                        强化学习优化：结合强化学习（如PPO、DDPG）进行参数优化，动态调整预测策略，增强模型适应性。

                        4.1.2 实时预测与反馈闭环

                        构建高效的实时预测框架，支持流式数据处理，并通过强化学习和自监督学习优化模型反馈机制。

                        Transformer-based 预测模型：采用时序Transformer（如Informer、TimeGPT）进行多步预测，提高长期预测的准确度。

                        增量学习：基于联邦学习和持续学习方法，使模型能够在不丢失历史信息的情况下适应新数据。

                        自适应反馈系统：建立闭环反馈架构，利用实时误差调整模型参数，提高系统的稳健性。

                        知识蒸馏：利用大模型进行知识压缩，将复杂的预测模型蒸馏到轻量级模型，以便在边缘设备上高效运行。

                        4.2 性能优化策略

                        4.2.1 算法并行化与高性能计算

                        多核与GPU加速：采用分布式计算（如Ray、Dask）和CUDA优化（如TensorRT）提升计算效率。

                        混合精度训练：使用FP16/INT8量化技术，在不显著降低精度的情况下加速大模型推理。

                        大模型剪枝与蒸馏：利用模型剪枝（如LTH，Lottery Ticket Hypothesis）减少计算复杂度，提高推理速度。

                        4.2.2 内存管理与资源优化

                        高效数据加载：采用Hugging Face Datasets等高性能数据管道，实现数据流式处理。

                        智能缓存机制：结合Redis、Apache Arrow等内存优化技术，减少IO瓶颈，提高吞吐量。

                        自动资源调度：基于Kubernetes和MLOps（如Kubeflow）动态分配计算资源，提升系统稳定性。

                        4.3 模型评估与监控

                        4.3.1 先进的评估与对抗测试

                        多维评估指标：全面衡量模型性能（RMSE、MAE、MAPE）并引入时间序列专用指标（如DTW、N-Wavelet RMSE）。

                        对抗鲁棒性评测：基于Adversarial ML技术，构建对抗样本测试模型的抗干扰能力。

                        回测与仿真：在历史数据上进行滚动回测，模拟真实环境下的模型表现。

                        4.3.2 实时监控与智能告警

                        可视化监控平台：集成Prometheus+Grafana，实时跟踪模型在生产环境中的表现。

                        自适应异常检测：结合Anomaly Detection Transformer（AD-Transformer）检测预测异常，并触发告警与自恢复机制。

                        模型漂移检测：采用KL散度、MMD等方法监控数据分布变化，及时调整模型。

                        4.4 可扩展性与兼容性

                        4.4.1 模块化架构与API设计

                        插件化模型架构：采用MLflow和ONNX格式管理模型，支持热更新。

                        标准化API接口：提供RESTful API和gRPC支持，便于与企业系统集成。

                        4.4.2 跨平台适配与云端部署

                        跨云兼容性：支持AWS、GCP、Azure等云平台，采用容器化（Docker + Kubernetes）进行弹性部署。

                        多语言绑定：提供Python、Java、Rust等语言SDK，支持多种编程环境。

                        边缘计算优化：结合TinyML，使轻量模型在边缘设备上高效运行，实现云-端协同计算。
                    """
            ...
            "maintenance_support": PromptTemplate(
                ...
                template=(
                    """
                    {system_message}
                    请描述{product_name}的维护和支持策略，包括维护计划和支持渠道,不少于1000字。
    ...
    def process(self):
        ...
        func_names_json_prompt = f"""
            ...
            至少生16个及以上功能模块, 每个功能模块至少包含6个及以上的子功能模块。
            产品简述:
            {product_description}
...
if __name__ == "__main__":
    ...
    platforms = [
        {
            "docx_name": "康养数据管理及医图诊断模型训练系统技术需求文档.docx",
            "title": " 康养数据管理及医图诊断模型训练系统技术需求文档",
            "product_name": "康养数据管理及医图诊断模型训练系统"
        }
