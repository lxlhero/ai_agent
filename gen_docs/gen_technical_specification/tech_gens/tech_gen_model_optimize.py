...
class DataPreprocessingModuleSpecGenerator:
    def __init__(self, overview_path, docx_name, title, product_name):
        ...
        self.system_message = """
            ...
            5. 特别注意，模型本身已经存在，所以目的是去调优其性能，尽量从升级调优、能力优化的角度思考，因为本产品是关于大模型优化升级的，而不是新模型的开发，所以这点非常重要！
        ...
        self.prompts = {
            ...
            "technical_specification": PromptTemplate(
                ...
                template=(
                    ...
                    实现方案：提供高层次的实现细节，包括使用的技术框架、算法或工具, 具体到版本和算法的设计细节，例如损失函数、网络结构、训练方法等。
                    ...
                    以下是一个精简版示例，这里假设是第4段落，仅供参考，具体子目录要看{index}是多少, 注意，子目录一定要生成:
                    4.1 算法集成与优化
                    4.1.1 时间序列分析算法包
